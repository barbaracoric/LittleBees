...
class Details_psn extends Component {
    static contextType = DataContext;
    state = {
        psn: this.context.psnn
    }

    getPsn = () => {
        if(this.props.match.params.id){
            const res = this.context.psnn;
            const data = res.filter(item =>{
                return parseInt( item._id ) === parseInt( this.props.match.params.id );
            })
            this.setState({psn: data})
        }
    }
                    ...
                    psn.map(item => (
                        <div className={theme ? "theme-details" : "details"} key={item._id}>
                            ...
                            <div className="title-details">
                                <div className="game-name"><p>{item.title} (PSN)</p></div>
